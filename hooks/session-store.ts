...
function reduceStats(stats: SessionData["sessionStats"], e: MonitoringEvent): SessionData["sessionStats"] {
  switch (e.eventType) {
    ...
    case "face_detection":
      if ((e as any).data?.reason === "no_face") {
        return { ...stats, noFaceIncidents: stats.noFaceIncidents + 1, violationCount: stats.violationCount + 1 }
      }
      if ((e as any).data?.reason === "multiple_faces") {
        return { ...stats, multipleFaceIncidents: stats.multipleFaceIncidents + 1, violationCount: stats.violationCount + 1 }
      }
      return stats
    case "gaze_tracking": {
...
function reducer(state: SessionData, action: Action): SessionData {
  switch (action.type) {
    ...
    case "SET_FIELDS": {
      // Non-derived fields replacement; keep snapshots/events bounded
